...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function filterJobs() {
    ...
    jobCards.forEach(card => {
      ...
      const location = card.querySelector('.bi-geo-alt').parentElement.textContent.toLowerCase().trim();
      const department = card.querySelector('.bi-briefcase').parentElement.textContent.toLowerCase().trim();

      let showCard = true;

      // Check location filter
      if (selectedLocation !== 'all locations' && !location.includes(selectedLocation)) {
        showCard = false;
      }

      // Check department filter
      if (selectedDepartment !== 'all departments' && !department.includes(selectedDepartment)) {
        showCard = false;
      }
